import type { IPluginManagerConfig } from "./utils/manager";
import { PluginManager } from "./utils/manager";
...
export async function initializePluginManager(
	config: Partial<IPluginManagerConfig> = {},
) {
...
function setupPluginManagerEvents() {
	...
	pluginManager.on("plugin:error", (instance, error) => {
		console.error(
			`[pluginManager] Plugin error: ${instance.config.name}`,
			error,
		);
	});
}
...
export function getPluginManager(): PluginManager {
	if (!pluginManager) {
		throw new Error(
			"PluginManager not initialized. Call initializePluginManager() first.",
		);
	}
	return pluginManager;
}
	...
	try {
		...
		for (const plugin of loadedPlugins) {
			try {
				...
				console.error(
					`[pluginManager] Failed to activate plugin ${plugin.config.name}:`,
					error,
				);
			}
		}
...
export async function activatePlugin(instanceId: string) {
	try {
		...
		console.error(
			`[pluginManager] Failed to activate plugin ${instanceId}:`,
			error,
		);
		throw error;
	}
...
export async function deactivatePlugin(instanceId: string) {
	try {
		...
		console.error(
			`[pluginManager] Failed to deactivate plugin ${instanceId}:`,
			error,
		);
		throw error;
	}
