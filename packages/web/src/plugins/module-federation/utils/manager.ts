...
export class PluginManager extends EventEmitter<IPluginManagerEvents> {
	...
	private cache = new Map<
		string,
		{ data: IPluginConfig[]; timestamp: number }
	>();
	...
	async activatePlugin(instanceId: string): Promise<void> {
		...
		if (instance.isActivated) {
			console.log(
				`[PluginManager] Plugin ${instance.config.name} is already activated`,
			);
			return;
		}
		...
		try {
			...
			console.log(
				`[PluginManager] Plugin ${instance.config.name} activated successfully`,
			);
		} catch (error) {
			console.error(
				`[PluginManager] Failed to activate plugin ${instance.config.name}:`,
				error,
			);
			throw error;
		}
	...
	async deactivatePlugin(instanceId: string): Promise<void> {
		...
		if (!instance.isActivated) {
			console.log(
				`[PluginManager] Plugin ${instance.config.name} is not activated`,
			);
			return;
		}
		...
		try {
			console.log(
				`[PluginManager] Deactivating plugin ${instance.config.name}`,
			);
			...
			console.log(
				`[PluginManager] Plugin ${instance.config.name} deactivated successfully`,
			);
		} catch (error) {
			console.error(
				`[PluginManager] Failed to deactivate plugin ${instance.config.name}:`,
				error,
			);
			throw error;
		}
	...
	private unregisterContributions(pluginId: string, _contributes: any): void {
		// 这里可以实现注销逻辑
		console.log(
			`[PluginManager] Unregistering contributions for plugin: ${pluginId}`,
		);
	}
	...
	async loadPlugins(instanceIds: string[]): Promise<IPluginInstance[]> {
		...
		return Promise.allSettled(loadPromises).then((results) =>
			results
				.filter(
					(result): result is PromiseFulfilledResult<IPluginInstance> =>
	...
	async destroy(): Promise<void> {
		...
		for (const instance of instances) {
			try {
				...
				console.error(
					`Failed to destroy plugin ${instance.instanceId}:`,
					error,
				);
			}
		}
